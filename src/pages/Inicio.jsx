...
const metricas = [
  { label: 'Cotizaciones abiertas', valor: 0, sub: 'Sin confirmar',   color: '#185FA5', bg: '#E6F1FB', icono: FileText  },
  { label: 'Órdenes de corte',      valor: 0, sub: 'Pendientes hoy',  color: '#854F0B', bg: '#FAEEDA', icono: Scissors  },
  { label: 'Productos en catálogo', valor: 6, sub: 'Tipos de piedra', color: '#2D4A2D', bg: '#EAF3DE', icono: Package   },
  { label: 'Retazos disponibles',   valor: 0, sub: 'En el patio',     color: '#2a2a2a', bg: '#f5f0e8', icono: Grid      },
]

const accesos = [
  { label: 'Nueva cotización', desc: 'Crear para un cliente',   icon: FileText, bg: '#EAF3DE', color: '#3B6D11' },
  { label: 'Nuevo cliente',    desc: 'Registrar en el sistema', icon: UserPlus, bg: '#E6F1FB', color: '#185FA5' },
  { label: 'Orden de corte',   desc: 'Asignar a cortador',      icon: Scissors, bg: '#FAEEDA', color: '#854F0B' },
]
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '10px' }}>
            {accesos.map((a, i) => {
              const Icon = a.icon
              return (
                <div key={i} style={{ backgroundColor: '#fff', borderRadius: '10px', border: '1px solid #e0d8c8', padding: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', transition: 'transform 0.15s' }}
